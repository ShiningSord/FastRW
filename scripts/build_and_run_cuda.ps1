...
param(
    [string]$Config = '',
    [ValidateRange(1, 2147483647)][int]$NumSamples = 2,
    [int]$ThreadsPerBlock = -1,
    [int]$CudaArchitecture = 86
)

$ErrorActionPreference = 'Stop'
Write-Warning 'Experimental Windows/CUDA support: not rigorously tested by the project maintainers; for reference only.'
...
$RunDir = Join-Path ([IO.Path]::GetFullPath((Join-Path $configDir $outputSetting))) 'cuda-experimental'
...
if ($matches.Count -gt 0) {
    $deviceMatch = [regex]::Match($text, 'CUDA device: (.+) \(compute capability')
    $gpu = if ($deviceMatch.Success) { $deviceMatch.Groups[1].Value } else { 'unknown' }
    ...
    $randomWalk = [pscustomobject]@{
        backend = 'cuda'
        validation_status = 'experimental; not rigorously tested by project maintainers'
        device = $gpu
